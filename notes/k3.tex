...
\begin{defn}
	A bialgebra is a \textit{\textbf{Hopf algebra}} when it is also equipped with an antipode map such that the following diagram commutes
			\[\begin{tikzcd}
			H\otimes H\otimes H \arrow[r,"m\otimes \operatorname{id}"]\arrow[d,"\operatorname{id}\otimes m"]&H\otimes H\arrow[d,"m"]\\
			H\otimes H\arrow[r,"m"]&H
		\end{tikzcd}\]
	\item[\textbf{A2}] Unitality.
		\[\begin{tikzcd}
			H\cong H\otimes \mathbb{C}\arrow[r,"\opertatorame{id}\otimes u"]\arrow[rr,bend right,"\opertatorame{id}",swap]&H\otimes H\arrow[r,"m"]&H
		\end{tikzcd}\]
\end{defn}
...
\begin{defn}
	...
	\[A^\bullet  \supset  F_1A^\bullet\supsetF_2A^\bullet\supset\ldots\]
	such that
