...
\begin{thing6}{Corollary 2}\leavevmode
...
\begin{thing5}{Theorem 2}\leavevmode
...
Also remember Lengendre's theorem that Pelle equation has solution. Also remember that two lattices of the same rank have commesurable isometry groups. And the other case when lattices are not of the same rank, so the group of isometries that can be extended from the small to the large is finite index in the large.
...
\end{coro}
where
\begin{prop}[local Torelli theorem for complex structures]\leavevmode
...
\begin{thm}[Nikulin-Morrison]\leavevmode
...
\begin{prop}[Teorema de Bertin]\leavevmode
...
\begin{thm}[Lefschetz hyperplane section throem]\leavevmode
...
\begin{remark}[This might be a big result]\leavevmode
...
\begin{remark}[Very basic]\leavevmode
...
\begin{claim}[Second year undergrad]\leavevmode
...
\subsection{The pluri-Laplacian and the Hessian}
...
First step toward proving local Torelli. Typically this lasts 3-lectures.
...
\begin{thm}[Local Torelli]\leavevmode
...
\begin{lemma}[Moser's trick]\leavevmode
...
$\mathsf{OK}$ so $\mathbb{P}\operatorname{er}$ is a smooth submersion. So the fibers are families of forms, which by Moser lemma we can pullback to one an another. {\color{3}So? When we collapse the identity component… a local bijection?}
...
\begin{thm}[C-symplectic Moser lemma]\leavevmode
...
\begin{thing8}{Corollary}[Local Torelli for C-symplectic]\leavevmode
...
\subsection{Bochner's vanishing}

\begin{thm}[Bochner vanishing]\leavevmode
...
\section{Class 21: Kähler cone and hyperkähler period space}
...
\subsection{Hyperkähler metrics, induced complex structure}
...
\begin{thm}[Chow-Raschevskii, 1938, 1939]\leavevmode
...
\begin{defn}[Gromov? \textit{Structures on Riemannian and non-Riemannian. .. ?} ]\leavevmode
